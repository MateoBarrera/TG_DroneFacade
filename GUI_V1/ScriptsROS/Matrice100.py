...
class Matrice100Services:
    ...
    def waypoint_mission(self, coord):
        ...
        for coordinate in coord.itertuples():
            wp = self.set_waypoints(coordinate[0],coordinate[1],coordinate[2])
            wps.append[wp]
...
def main():
    ...
    service.setLocalPosition()
    position = rospy.Subscriber('dji_sdk/local_position', PointStamped)
    print(position.header)
    print(position.point.x,position.point.y,position.point.z)
    """ coordenadas = pd.DataFrame(columns=['latitude','longitude','altitude'],)
    coordenadas = coordenadas.append({'latitude': position.y, 'longitude': position.x, 'altitude': position.z+12 },ignore_index=True)
    coordenadas = coordenadas.append({'latitude': position.y+0.00030, 'longitude': position.x, 'altitude': position.z+12 },ignore_index=True)
    coordenadas = coordenadas.append({'latitude': position.y+0.00030, 'longitude': position.x+0.00020, 'altitude': position.z+12 },ignore_index=True)
